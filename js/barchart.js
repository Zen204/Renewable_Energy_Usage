function barchart(width, height, id) {
    ...
    // Adjust the padding of the x scale
    ...
    function update(data) {
        ...
        enter.selectAll("rect")
            ...
            .transition()
            .duration(750) // Add transition duration to make changes visible
            .attr("y", d => y(d.value))
        ...
        bars = bars.enter().merge(bars)
            .attr("transform", d => `translate(${x(d.Country)},0)`); // Ensure proper positioning of all bars
    }

    // Load and filter data for initial display
    d3.csv("data/full_file.csv", function(data) {
        var initialData = data.filter(d => d.Country === "World"); // this sets world as the default
...
function updateBarChart(selectedCountries) {
    d3.csv("data/full_file.csv", function(data) {
        ...
        if (selectedCountries.length === 0) {
            filteredData = data.filter(d => d.Country === "World"); // Only show World as default view
        } else {
